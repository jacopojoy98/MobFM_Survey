...
from utils import train_stop_ddp, evaluate_ddp
...
def test_ddp():
    ...
    parser.add_argument("--city", nargs='+', default=['Chicago','Seattle', 'WashingtonDC','LosAngeles','NewYork'])
    parser.add_argument("--target_city", nargs='+', default=['Chicago','Seattle', 'WashingtonDC','LosAngeles','NewYork'])
    ...
    for city in args.target_city:
        city = [f'{city}']
        test_dataset = TrajDataset(args.test_root, city, args.B, args.T, args.few_shot)
        test_sampler = DistributedSampler(test_dataset)
        test_loader = DataLoader(test_dataset, batch_size=args.B, sampler=test_sampler)
        evaluate_ddp(model, test_loader, log_dir, args.B, city, device)
...
if __name__ == "__main__":
    test_ddp()
    # CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc-per-node=4 main_ddp.py