...
def analyze_trainable_params(model):
    print("=== Analyzing Model Parameter Trainable Status ===\n")

    # Statistics
    total_params = 0
    trainable_params = 0

    llm_total, llm_trainable = 0, 0
    vision_total, vision_trainable = 0, 0
    projector_total, projector_trainable = 0, 0
    ...
    for name, param in model.named_parameters():
        ...
        # Language model part
        if name.startswith("model.model") or name.startswith("base_model.model"):
            llm_total += num_params
            if param.requires_grad:
                llm_trainable += num_params

        # vision_tower part
        elif "vision_tower" in name:
            vision_total += num_params
            if param.requires_grad:
                vision_trainable += num_params

        # mm_projector part
        elif "mm_projector" in name:
            projector_total += num_params
            if param.requires_grad:
                projector_trainable += num_params

        # LoRA parameter detection
        if isinstance(getattr(param, "lora_layer", None), LoraLayer) or "lora" in name.lower():
            lora_param_count += num_params

    def fmt(x): return f"{x/1e6:.2f}M"

    print(f"Total model parameters: {fmt(total_params)}")
    print(f"Trainable model parameters: {fmt(trainable_params)}\n")

    print("▶️ Language Model (LLM) Part:")
    print(f"    Total params: {fmt(llm_total)} | Trainable: {fmt(llm_trainable)}")
    if llm_trainable > 0:
        if lora_param_count > 0:
            print("    ✅ Using LoRA for tuning")
        else:
            print("    ✅ Performing full parameter tuning")
    else:
        print("    ❌ Frozen")

    print("\n▶️ Vision Tower Part:")
    print(f"    Total params: {fmt(vision_total)} | Trainable: {fmt(vision_trainable)}")
    print("    ✅ Trainable" if vision_trainable > 0 else "    ❌ Frozen")

    print("\n▶️ MM Projector Part:")
    print(f"    Total params: {fmt(projector_total)} | Trainable: {fmt(projector_trainable)}")
    print("    ✅ Trainable" if projector_trainable > 0 else "    ❌ Frozen")
...
def print_gpu_usage(tag=""):
    device = torch.cuda.current_device()  # The GPU card where the current process is located
    allocated = torch.cuda.memory_allocated(device) / 1024 ** 2
    reserved = torch.cuda.memory_reserved(device) / 1024 ** 2
    print(f"[{tag}] Rank {os.environ.get('RANK', '?')} | GPU {device} | Allocated: {allocated:.2f} MB | Reserved: {reserved:.2f} MB")


local_rank = int(os.environ.get("LOCAL_RANK", 0))
setproctitle(f"llava{local_rank}")
...
class DataArguments:
    ...
    data_path_eval: Optional[str] = field(default=None)
    image_folder_eval: Optional[str] = field(default=None)
...
def preprocess_llama3(
    ...
    # system_message = "You are an intelligent user mobile assistant that can infer user behavior preferences based on the embedding of the historical behavior sequence of mobile activities. This embedding is generated by a sequence model encoder, which takes as input the raw historical behavior sequence of user mobile activities. Each element in the raw sequence includes the following information: the name of the event, the time it occurred (day of the week, the specific 15-minute time slot within the day, meaning the time resolution is 15 minutes), and the digital ID of the location where it occurred. First, understand the mobile historical behavior embedding sequence generated by the sequence model encoder. Your understanding of the behavior sequence needs to be at a level that can assist with subsequent tasks such as behavior prediction and behavior pattern discovery. Then, based on my specific instructions and your understanding, answer questions accordingly."
        ...
        for conv in source:
            ...
            if content is None:
                ...
                if role in ["user", "system"]:
                    ...
                    # print(encode_id,len(encode_id),'-------------')
                    # decoded_text = tokenizer.decode(encode_id, skip_special_tokens=False)
                    # print('gpt',decoded_text,'decoded_text')
...
def preprocess_qwen2(
    ...
    for i, source in enumerate(sources):
        ...
        if task_type is not None:
            task_type_sample.append(task_type)
        # print(tokenizer.convert_tokens_to_ids("\n\n"),'271')
        ...
        targets.append(target)
        if task_type is not None:
            task_type_batch.append(task_type_sample)
    ...
    if task_type is not None:
        task_type_batch = torch.tensor(task_type_batch, dtype=torch.long)
        # print(input_ids.shape,targets.shape,task_type_batch.shape) #right
        return dict(
            input_ids=input_ids,  # tensor(bs x seq_len)
            labels=targets,  # tensor(bs x seq_len)
            task_type_batch = task_type_batch
        )
    else:
        return dict(
            input_ids=input_ids,  # tensor(bs x seq_len)
...
def preprocess_plain_llama3(
    ...
    assistant_tag_len = len([128006,  78191, 128007,271])
    # IMAGE_TOKEN_INDEX
    user_input_ids = [128000, 128006,    882, 128007,    271,  128256, 128009]
...
#         # conversation = source[0]['value'] + source[1]['value'] + conversation_lib.default_conversation.sep + tokenizer.eos_token 
#         # conversation = source[0]['value'] + source[1]['value'] +' '+ tokenizer.eos_token 

#         conversations.append(conversation)
#     # tokenize conversations
#     # print(conversations,'prompt')
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "qwen2":
        # print('chose qwen2')
        return preprocess_qwen2(sources, tokenizer, has_image=has_image,system_message=system_message,task_type=task_type)
    # pr(123)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if eval_dataset:
            ...
            list_data_dict = json.load(open(data_args.data_path_eval, "r"))
            list_data_dict = list_data_dict[::5]
            # if len(list_data_dict)>=10000:
            #     list_data_dict = list_data_dict[::2]

        else:
            list_data_dict = json.load(open(data_path, "r"))
            "all_task"
            list_data_dict = list_data_dict[0:60000] +  list_data_dict[120000:180000] +  list_data_dict[180000:180000+18934*2+20682] + list_data_dict[180000+18934*2+31537:]
        ...
        # print(image_token_index,"qwen2_image_token_index not match DEFAULT_IMAGE_Index") #151665
        # assert image_token_index == 128256, "llama3_image_token_index not match DEFAULT_IMAGE_Index"  # FIXME
        # assert image_token_index == 151665, "llama3_image_token_index not match DEFAULT_IMAGE_Index"  # FIXME
        assert image_token_index == IMAGE_TOKEN_INDEX, "llama3_image_token_index not match DEFAULT_IMAGE_Index"  # FIXME

        if eval_dataset:

            self.llava_pretrain_seq_data = np.load(self.data_args.image_folder_eval)

        else:
            self.llava_pretrain_seq_data = np.load(self.data_args.image_folder) 

            # loc = self.llava_pretrain_seq_data[:,:,2]
            # print(loc[loc>24434])
            # print(np.max(loc))
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'seq' in sources[0]:
            ...
            try:
                ...
                seq = torch.tensor(self.llava_pretrain_seq_data[0,-layer_num:,:])

            # if  24448 in seq[:,2]:
            #     print(i,'train >24434',seq[:,2])
        ...
        if isinstance(i, int):
            ...
            if task_type is not None:
                data_dict = dict(input_ids=data_dict["input_ids"][0],
                            ...
                            task_type_batch =data_dict["task_type_batch"][0] )
            else:
                data_dict = dict(input_ids=data_dict["input_ids"][0],
                             labels=data_dict["labels"][0])
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # print(instances[0].keys(),"instances[0]")
        if 'task_type_batch' in instances[0]:
            task_type_batch = torch.tensor([instance['task_type_batch'] for instance in instances])
        else:
        ...
        # print(task_type_batch,"task_type_batch")
        if task_type_batch is not None:
            batch['task_type_batch'] = task_type_batch
            # print(input_ids.shape,labels.shape,task_type_batch.shape) 
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        "Note that the device cannot be specified here, as it will conflict with the device automatically allocated by deepspeed"
    ...
    lora_model_path = '/save_model/0820_stage0_pretrain_load_7b_100wan_gai_right_goon_9000'
    ...
    mm_projector_path = "/llava-0820_stage1_best_model-pretrain/mm_projector.bin"
    ...
    for name, param in model.named_parameters():
        # Assume LoRA parameter names contain "lora", adjust according to your model
    ...
    for p in model.get_model().mm_projector.parameters():
        p.requires_grad = True
    # for p in model.get_model().mm_time_projector.parameters():
    #     p.requires_grad = True
    ...
    if True:
        ...
        # (If you keep HF/DS's --bf16 True)
        # Disable autocast for visual forward pass, then align output features to LLM dtype (usually BF16)
    ...
    print(f"Trainable parameters: {trainable_params/1e6:.1f}M | Frozen parameters: {frozen_params/1e6:.1f}M")
    # # # # done modify by reference
    "Do not comment out"
    ...
    print(tokenizer.convert_tokens_to_ids('<image>'))
    print(tokenizer.convert_tokens_to_ids('<sequence>'))
    print(tokenizer.convert_tokens_to_ids(DEFAULT_IMAGE_TOKEN))
    ini_tokenizer_len = len(tokenizer)
    print(ini_tokenizer_len,'ini_tokenizer_len')
    print(model.get_vision_tower().vision_tower.lm_head.weight.dtype,'vision_tower.dtype---after')
    print(model.get_model().dtype,'model.dtype---after')
    ...
    # ini-llava-save used in the feature alignment stage-----------------------------
    model.config.use_cache = True


    return 0 
    def get_best_check_points(path):
        # Add try-sleep and then use a temp to try
        try:
        ...
        eval_steps = []
        eval_losses =[]
        for item in data['log_history'][:-1]:
            if "eval_loss" in item:
                eval_steps.append(item['step'])
                eval_losses.append(item['eval_loss'])
        loss_step_dict = {loss: step for loss, step in zip(eval_losses, eval_steps)}

        # Find the lowest loss
        min_loss = min(loss_step_dict.keys())

        # Output the step corresponding to the lowest loss
        min_loss_step = loss_step_dict[min_loss]

        # data["train_batch_size"] = 1
        # data["max_steps"] = min_loss_step


        return min_loss_step

    best_step = get_best_check_points(training_args.output_dir)
    ...
    trainer.args.max_steps = best_step -1
    # trainer.args.per_device_train_batch_size = 1
    # print(training_args.output_dir+"/checkpoint-"+str(best_step))
    trainer.train(resume_from_checkpoint=training_args.output_dir+"/checkpoint-"+str(best_step))
    # trainer.train(resume_from_checkpoint="/llava-0820_stage2_all_task_4_city_no_sequence_emb_true-finetune/checkpoint-2300")


    print(trainer.args.max_steps)

    def save_model_with_deepspeed(model, save_directory):
        ...
        for name, param in model.named_parameters():
            param.data = maybe_zero_3(param, name=name)
        # Save the model
        model.save_pretrained(save_directory)
    output_model_path = "/save_model/0820_stage2_all_task_4_city_no_sequence_emb_true_goon"+"_"+str(best_step)
    ...
    trainer.save_checkpoint_mm(mm_projector_path)

    # mm_projector_path = os.path.join(output_model_path, "mm_time_projector.pth")
    # trainer.save_checkpoint_mm_houxuan(mm_projector_path)

    save_model_with_deepspeed(model,output_model_path) 
